...
def evaluate(model, base_test, trig, cfg, device):
    ...
    # dose-response: predictions at a grid of doses, displacement vs clean prediction
    ...
    Pd, Td, Tg, _ = _predict(model, dl(tds), device)
    res['asr@ref'] = M.attack_success(Pd, Pc, Td, pivot, dref,
                                      theta_max_deg=cfg['theta_max_deg'])
    return res
